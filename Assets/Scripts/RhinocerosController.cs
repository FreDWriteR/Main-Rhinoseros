...
{
    [SyncVar]
    public Color RhinocerosColor = Color.white; //Синхронизация цвета игрока
    ...
    public TextMeshPro RhinocerosName; //Имя игрока над головой
    public TextMeshProUGUI RhinocerosNameUI; //Имя игрока на канве экрана
    public TextMeshProUGUI CountPuncture; //Количество попаданий по другим игрокам
    public TextMeshProUGUI MainRhinoceros; //Отображение победителя
        ...
        if (Input.GetKeyDown(jerkKey))
        {
            ...
            if (!moveDirection.Equals(Vector3.zero))
            {
                ...
                while (DirectionJerk.magnitude < jerkDistance)
                {
                    ...
                    DirectionJerk = RhinocerosStartPosition - gameObject.transform.position; //Вычисляем расстояние пройденное с момента начала рывка и сообщаем его серверу
